...
pub fn generate_back(back_path: String) -> String {
    let img = image::Image::new(&back_path);

    let color_back = image::to_color_table(
        &img,
    ...
    let diffrent = image::diffrent(template_back, color_back);
    let res = BACK_DIRECTIVES.to_string() + &directives::to_replace(diffrent, false);
    res
}
